...
void Model::setListOfImages(QList<QPixmap> *listPixmaps, QList<int> *listPixmapsIndex)
{
    beginResetModel();
    delete tree;
    this->listPixmaps       = listPixmaps;
    this->listPixmapIndex   = listPixmapsIndex;
    if(listPixmaps==NULL) {
        this->rc    = 0;
        tree        = new QVector<Node>(0, Node(0));
    } else {
        this->rc    = listPixmaps->size();
...
void Model::refreshListOfImages()
{
    beginResetModel();
    delete tree;
    this->rc    = this->listPixmaps->size();
    tree        = new QVector<Node>(this->rc, Node(0));
    endResetModel();
}

Model::Model(QList<QPixmap>* listPixmaps, QList<int>* listPixmapsIndex, QObject *parent)
    : QAbstractItemModel(parent),
      services(QPixmap(":/images/services.png")),
      rc(listPixmaps->size()), cc(1),
      tree(new QVector<Node>(listPixmaps->size(), Node(0)))
{
    this->listPixmaps       = listPixmaps;
    this->listPixmapIndex   = listPixmapsIndex;
}
...
QVariant Model::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole) {
        return QString("%1/%2/%3").arg(listPixmapIndex->at(index.row())).arg(index.row()+1).arg(rc);
    }
    if (role == Qt::DecorationRole) {
        return listPixmaps->at(index.row());
    }
    return QVariant();
}
